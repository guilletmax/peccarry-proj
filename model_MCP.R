...
DRY <- 1
WET <- 2
#move inside function for sensitivity analysis
...
simulate_movement <- function(x_length, y_length, count_forest, percent_forest, 
                             steps, iter) {
#  simulate_movement <- function(count_forest, percent_forest, 
 #                                max_dist, depletion_rate,
  #                              stuck_cutoff, patch_regrowth) {  
  #include parameters outside sensitivity analysis
#  x_length<-167
#  y_length<-167
#  steps<-2800
#  iter<-1
  ...
  #include fixed parameters as variables for sensitivity analysis
#  DEPLETION_LEVEL_CUTOFF <- depletion_rate
#  STUCK_TIMER <- stuck_cutoff
#  MAX_CROSSING_DISTANCE <- max_dist
  PATCH_REGROWTH <- 30
  ...
  simulate_movement <- function() {
    ...
    for (i in 1:steps) {
      ...
      #add that every 30 value to sensitivity analysis
      #if(i %% 30 == 0) {
      if(i %% PATCH_REGROWTH == 0) {  
  ...
  nums <- c(0:max_tread)
  freq_all <- plyr::count(as.vector(move_grid))
  freq_0 <- freq_all[1,2]
  ...
  write.csv(consecutive_stuck_counter, file = paste("consecutive_stuck_counter", percent_forest, iter, ".csv", sep = "_"))
  
  outputs<-c(freq[2,1], crossed_matrix_counter, total_distance, length(consecutive_stuck_counter), max_dist_bw_forests, 
    depleted_counter, max(consecutive_stuck_counter), non_depleted_counter)
  #outputs<-c(crossed_matrix_counter, total_distance, length(consecutive_stuck_counter), max_dist_bw_forests, 
   #          depleted_counter, max(consecutive_stuck_counter), non_depleted_counter)
 ...
 # return(c(freq[2,1], crossed_matrix_counter, total_distance, length(consecutive_stuck_counter), max_dist_bw_forests, 
  #         depleted_counter, max(consecutive_stuck_counter), non_depleted_counter)) 
  return(outputs)
}
